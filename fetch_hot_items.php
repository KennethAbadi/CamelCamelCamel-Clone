...
if($cat != 'all') {
    $sql = "SELECT * FROM products JOIN tracking ON products.id=tracking.id WHERE category='$cat' ORDER BY clicks DESC";
} else {
    $sql = 'SELECT * FROM products JOIN tracking ON products.id=tracking.id ORDER BY clicks DESC';
}
